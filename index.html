    ...
    if(canDelete){
      if(mine){
        return `<div class="bubble-row own">
          <button class="bubble-menu-btn" onclick="toggleBubbleMenu(event,'${m.id}','kelas',${isOwn})">⋮</button>
          ${bubbleHtml}
        </div>`;
      }
...
/* ============================================================
   MENU TITIK TIGA (⋮) DI BUBBLE PESAN — edit/hapus pesan
============================================================ */
let bubbleMenuTargetId = null, bubbleMenuTargetType = null;
function toggleBubbleMenu(ev, id, type, canEdit){
  ev.stopPropagation();
  ...
  bubbleMenuTargetId = id; bubbleMenuTargetType = type;
  ...
  menu.innerHTML = (canEdit?'<button onclick="handleBubbleEdit()">✏️ Edit</button>':'') + '<button onclick="handleBubbleDelete()">🗑️ Hapus</button>';
  document.body.appendChild(menu);
}
document.addEventListener('click', ()=>{ document.getElementById('bubbleDropdown')?.remove(); });
function handleBubbleEdit(){
  const id = bubbleMenuTargetId, type = bubbleMenuTargetType;
  document.getElementById('bubbleDropdown')?.remove();
  if(type==='kelas') editChatMessage(id);
  else if(type==='internal') editInternalChatMessage(id);
}
function handleBubbleDelete(){
  const id = bubbleMenuTargetId, type = bubbleMenuTargetType;
  document.getElementById('bubbleDropdown')?.remove();
  if(type==='kelas') deleteChatMessage(id);
  else if(type==='internal') deleteInternalChatMessage(id);
}
...
function renderInternalChat(rows){
  ...
  box.innerHTML = rows.map(m=>{
    ...
    if(canDelete){
      if(isMe){
        return `<div class="bubble-row own">
          <button class="bubble-menu-btn" onclick="toggleBubbleMenu(event,'${m.id}','internal',${isMe})">⋮</button>
          ${bubbleHtml}
        </div>`;
      }
      return `<div class="bubble-row">
